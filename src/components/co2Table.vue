...
import type { Co2Record, SortKey } from '../types/co2'
    ...
    <div class="card-body">
      ...
      <div v-else class="table-responsive">
        <table class="table table-striped table-hover align-middle">
          <thead>
            <tr>
              <th scope="col" class="sortable" @click="setSort('entity')">
                Name <span v-if="sortKey === 'entity'">{{ sortDir === 'asc' ? '▲' : '▼' }}</span>
              </th>
              <th scope="col" class="sortable d-none d-md-table-cell" @click="setSort('category')">
                Kategorie <span v-if="sortKey === 'category'">{{ sortDir === 'asc' ? '▲' : '▼' }}</span>
              </th>
              <th scope="col" class="sortable d-none d-lg-table-cell" @click="setSort('sector')">
                Sektor <span v-if="sortKey === 'sector'">{{ sortDir === 'asc' ? '▲' : '▼' }}</span>
              </th>
              <th scope="col" class="sortable" @click="setSort('year')">
                Jahr <span v-if="sortKey === 'year'">{{ sortDir === 'asc' ? '▲' : '▼' }}</span>
              </th>
              <th scope="col" class="sortable" @click="setSort('emissions')">
                Emissionen (tCO₂e) <span v-if="sortKey === 'emissions'">{{ sortDir === 'asc' ? '▲' : '▼' }}</span>
              </th>
            </tr>
          </thead>
          <tbody>
